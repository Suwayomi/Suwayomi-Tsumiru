...
**Decode-size cap (#196):** long-strip pages are ~800×15000 at the source; decoding them at native resolution uploads a ~48 MB texture per page that the compositor samples every frame while scrolling — the high-GPU cost on tall/narrow windows. `ServerImage` takes `memCacheWidth`/`memCacheHeight` and decodes at the on-screen pixel size (`maxContentWidth`/`context` axis × devicePixelRatio) instead. `cached_network_image` hands `imageBuilder` the RAW provider, so its `memCache*` is ignored whenever an `imageBuilder` is set (multichapter / infinity / paged) — `ServerImage` works around this by wrapping the caller's builder in `ResizeImage` (offline/crop go through the same `_capDecode`). Aspect is preserved, so the `MeasureSize`/scroll-anchor height contract is untouched (render-only, same category as `cropBorders`). Wired into the long-strip, infinity, and infinity-fallback readers; paged is left native (its pages aren't oversized strips).
