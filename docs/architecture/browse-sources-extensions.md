...
- **Applied browse filters:** `appliedSourceFilterProvider(sourceId)` (keep-alive, deliberately not persisted) holds the applied `List<FilterChange>` for the app session, matching WebUI's per-source session storage. The sheet is seeded from it on every open (`SourceMangaFilter.appliedChanges` + a `useEffect`, since the end-drawer instance can keep its hook state between opens), so submitting builds on the current selection rather than replacing it. Reset clears the provider and refreshes the paging controller.
...
- **Bulk add duplicate detection:** `source_manga_display_view.dart` runs `splitSelectionForDuplicates` (library domain's shared matcher, checking both the library and earlier picks in the same selection) before adding. Clean picks batch-add immediately; flagged picks resolve one at a time through the Komikku-style dialog queue (Add anyway / Allow all / Skip it / Skip all / Show entry / Cancel) — cancel stops remaining prompts, entries already added stay added. The closing snackbar's counts come from server-confirmed ids, not the requested selection. Single-add (`addMangaToLibraryWithCategory`) gates on the same matcher and, on a hit, can migrate the existing entry into the new one via the bulk migration runner instead of adding a duplicate.
...
- **`currentChanges` falls back to `const []`, not `[]`** — it is a `useEffect` key in `FilterGroupWidget`, and a fresh list every build would re-fire the effect on every build.
