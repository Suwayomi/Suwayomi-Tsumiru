...
Basic, Simple Login and no-auth servers keep existing downloads at an unclaimed `offline/` root. When `.account-root-id` belongs to a UI Login catalogue, they use `offline/non-account/` instead. That directory has its own catalogue stamp and last-verified identity/address keys (`offlineNonAccountCatalogServerIdKey`, `offlineNonAccountLastServerIdKey`, `offlineNonAccountLastServerAddressKey`); UI Login restores cannot overwrite them. Foreground admission, mismatch handling, catalogue clear and both workers use the effective identity keys from `offline_storage_identity.dart`. `offlineNonAccountScopedKey` persists this layout for the foreground service. The scheduled `CatchupWorkSpec.nonAccountScoped` field defaults to false for older snapshots and directs workers to the same directory. `isAccountStoragePath` defines the separate account-directory layout for session storage and the spec writer.

Storage validation records version `1` in `.storage-vetted` after a successful tree walk and atomic marker rename. Missing or outdated markers require validation; completed, vetted stores retain cheap ancestor, catalogue-sidecar and ownership checks on reopen without walking every page. This trusts app-controlled download trees after validation. Incomplete migrations revalidate their destination before completion. A failed validation keeps its file path in the recovery banner.

The On-device storage list includes retained login catalogues and inactive non-account downloads, with their sizes and removal actions. The active storage path cannot be removed. Removing non-account storage preserves other account trees and clears only its independent identity preferences. Account settings can remove a retained inactive catalogue. Removal clears its data while retaining the cleared marker and ownership lock files, so later login cannot import the old legacy root again or create a second lock beside one still in use. Identity changes, catalogue removal and download workers use the same ownership protocol. See `offline_runtime_storage.dart`, `account_catalogue_repository_io.dart` and `account_storage_migration_io.dart`.
...
Interrupted migration is deferred until the app is usable. `AccountSessionStorage.recover()` publishes progress and failure state for the shell banner, supports retry, and uses the same captured session for cancellation, failure publication and overlapping restores. A successful re-login to the same account cancels the old recovery; restore leaves it pending for automatic startup recovery under the new session. Catalogue reconciliation preserves both SQLite snapshots, merges missing records, and asks the user to choose conflicting reading state. Recovery comparisons and saved choices include reading timestamps and pending edits. Matching and imported rows are also recorded as reconciled, so a sync acknowledgement may clear pending flags without causing a new conflict on retry. Conflict choices show the reading timestamp and pending changes for each copy. Recovering legacy reading state copies its timestamp and pending flags; recovery does not enqueue already-synced progress as a new read. Recovery moves files on the same filesystem; fallback copying and duplicate verification run in a cancellable isolate. Originals are removed only after verified retention. A truncated catalogue backup survives an interrupted replacement and is removed after the replacement rename succeeds; divergent backup bytes are retained. Root removal excludes nested account folders and shared controls.
