...
void main() {
  ...
  group('mangaPreferredScanlatorsProvider effective value', () {
    test('new key wins when present', () async {
      final c = await seeded(_RecordingRepo(), [
        Fragment$MangaDto$meta(
          key: 'flutter_preferredScanlators',
          value: '["A","B"]',
        ),
    ...
    test('legacy sentinel means no preference', () async {
      final c = await seeded(_RecordingRepo(), [
        Fragment$MangaDto$meta(
          key: 'flutter_scanlator',
          value: 'flutter_scanlator',
        ),
      ]);
  ...
  group('setPreference', () {
    test(
      'writes new key as JSON, then mirrors rank-1 into legacy key',
      () async {
    ...
    test(
      'a failed write reports failure and publishes no local success',
      () async {
        ...
        final notifier = c.read(
          mangaPreferredScanlatorsProvider(mangaId: 1).notifier,
        );
