...
void main() {
  ...
  test(
    'setChapterProgress records progress + marks dirty; clear clears it',
    () async {
      await seed(5);
  ...
  test(
    'down-sync preserves dirty local progress (no stale-server overwrite)',
    () async {
  ...
  test(
    'down-sync applies a server bookmark even while a read is pending (#13)',
    () async {
      ...
      expect(
        c!.isBookmarked,
        true,
      ); // server bookmark propagated, not clobbered
  ...
  test(
    'clearProgressDirtyIfUnchanged keeps the flag when a newer write landed',
    () async {
      await seed(10);
  ...
  test(
    'partial write dirties position only; completion dirties both',
    () async {
      await seed(5);
  ...
  test(
    'setChapterReadState writes isRead + readStateDirty, not position',
    () async {
      await seed(5);
      await db.setChapterProgress(5, lastPageRead: 4, isRead: null);
