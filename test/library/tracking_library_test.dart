...
void main() {
  ...
  group('normalizedScore', () {
    ...
    test('AniList 0-100: score 70 on scaleMax 100 → 7.0', () {
      expect(
        normalizedScore(score: 70.0, scaleMax: 100.0),
        closeTo(7.0, 0.001),
      );
    });
    ...
    test('max score → 10.0', () {
      expect(
        normalizedScore(score: 10.0, scaleMax: 10.0),
        closeTo(10.0, 0.001),
      );
      expect(
        normalizedScore(score: 100.0, scaleMax: 100.0),
        closeTo(10.0, 0.001),
      );
    });

    test(
      'MangaUpdates fractional scale 0–9.9: last label "9.9" → scaleMax 9.9',
      () {
        ...
        expect(
          normalizedScore(score: 9.9, scaleMax: 9.9),
          closeTo(10.0, 0.001),
        );
        // score 4.95 should normalize to ~5.0.
        expect(
          normalizedScore(score: 4.95, scaleMax: 9.9),
          closeTo(5.0, 0.001),
        );
      },
    );
  ...
  group('MangaSort.trackerScore sort', () {
    test('ascending by mean normalized score', () {
      final mangas = [
        _manga(
          id: 1,
          trackNodes: [_trackNode(trackerId: 1, status: 1, score: 8.0)],
        ),
        _manga(
          id: 2,
          trackNodes: [_trackNode(trackerId: 1, status: 1, score: 5.0)],
        ),
        _manga(
          id: 3,
          trackNodes: [_trackNode(trackerId: 1, status: 1, score: 9.0)],
        ),
      ];
      final ids = _apply(
        mangas,
    ...
    test('untracked sorts last in ascending direction', () {
      final tracked = _manga(
        id: 1,
        trackNodes: [_trackNode(trackerId: 1, status: 1, score: 3.0)],
      );
  ...
  group('per-tracker filter', () {
    ...
    setUp(() {
      ...
      withT2 = _manga(
        id: 2,
        trackNodes: [_trackNode(trackerId: 2, status: 1, score: 5.0)],
      );
    ...
    test('true keeps only manga tracked by that tracker', () {
      final ids = _apply(
        [withT1, withT2, noTracker],
        sortedBy: MangaSort.alphabetical,
        trackerFilters: {1: true},
      );
      expect(ids, [1]);
    });
