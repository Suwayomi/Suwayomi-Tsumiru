...
class MigrationBulkRunScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final sameSession =
        !origin.store.sessionChanging &&
        origin.store.sessionEpoch == origin.epoch;
    ...
    return Scaffold(
      ...
      body: unavailable.value || !sameSession
          ? Emoticons(
              title: l10n.migrationIdentityUnavailable,
              button: TextButton(
                onPressed: () async {
                  ref.invalidate(serverInstanceIdProvider);
                  try {
